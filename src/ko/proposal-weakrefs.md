...
객체에 대한 약한 참조는 객체를 활성화하기에 충분하지 않습니다. 오직 남아있는 참조(즉, 약한 참조에 의해 참조되는 객체)가 약한 참조일 때 가비지 컬렉션은 자유롭게 참조를 파괴하고 메모리를 다른 용도로 재사용할 수 있습니다. 그러나, 객체가 실제로 파괴되기 전까지는 강력한 참조가 없더라도 약한 참조가 객체를 반환할 수 있습니다.
